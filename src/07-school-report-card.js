...
export function generateReportCard(student) {
  ...
  if (student !== null && typeof student === "object"){
    if (typeof student.name === "string" && student.name.trim() !== "" && student.marks && typeof student.marks === "object"){
      ...
      const passedSubjects = entries.filter(([sub, mark]) => mark >= 40).map(([sub]) => sub)
      
      const failedSubjects = entries.filter(([sub, mark]) => mark < 40).map(([sub]) => sub)
